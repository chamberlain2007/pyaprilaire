...
def package_logger():
    """Restore the package logger that `main` configures"""
...
def session(event_loop):
    session = DebugSession("localhost", 7001, auto_status=False)
...
def test_verbose_logging_goes_to_stderr(package_logger):
    cli._configure_logging(verbose=True, use_tui=False)

    handler = package_logger.handlers[-1]
...
@pytest.mark.parametrize(("verbose", "use_tui"), [(True, True), (False, False)])
def test_logging_is_otherwise_discarded(package_logger, verbose, use_tui):
    cli._configure_logging(verbose=verbose, use_tui=use_tui)

    assert isinstance(package_logger.handlers[-1], logging.NullHandler)
