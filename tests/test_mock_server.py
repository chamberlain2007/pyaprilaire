...
def test_the_sequence_number_wraps_within_the_thermostat_range(protocol):
    protocol.sequence = 255

    assert protocol._get_sequence() == 128
...
async def test_an_unreadable_attribute_is_nacked(protocol):
    (response,) = send(protocol, Action.READ_REQUEST, FunctionalDomain.STATUS, 2)

    assert isinstance(response, NackPacket)
    assert response.status_code == NackStatus.ATTRIBUTE_NOT_READABLE
