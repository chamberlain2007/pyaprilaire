...
def test_mapping_fields_excludes_padding():
    names = [
        field.name
        for field in mapping_fields(Action.READ_RESPONSE, FunctionalDomain.SETUP, 1)
    ]

    assert names[:2] == ["temperature_scale", "control_setup"]
...
def test_build_packet_leaves_missing_fields_null():
    packet = build_packet(
...
def test_build_packet_rejects_unknown_field():
    with pytest.raises(ValueError, match="'nope' is not a field"):
        build_packet(Action.WRITE, FunctionalDomain.CONTROL, 1, ["nope=1"])


def test_build_packet_rejects_non_pair():
    with pytest.raises(ValueError, match="is not a name=value pair"):
        build_packet(Action.WRITE, FunctionalDomain.CONTROL, 1, ["mode=1", "fan"])
...
def test_build_field_packet_leaves_missing_fields_null():
    packet = build_field_packet(
        Action.WRITE, FunctionalDomain.CONTROL, 1, {"mode": "3"}
    )

    assert packet.data[Attribute.MODE] == 3
    assert packet.data[Attribute.FAN_MODE] is None
...
def test_an_optional_parameter_is_parsed_as_its_type(commands):
    command = find_command("read_mac_address_and_wait", commands)

    (timeout,) = command.parameters

    assert timeout.annotation is int
    assert not timeout.required
    assert command.parse_arguments(["5"]) == [5]
    assert command.parse_arguments([]) == [None]


def test_parameters_that_are_not_a_single_type():
    def method(self, value: int | str, other): ...

    value, other = commands_module._describe_command("method", method).parameters

    assert value.annotation == int | str
    assert value.type_name == "int | str"
